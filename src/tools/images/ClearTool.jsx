...
import { Image as ImageIcon, Upload, Download, RefreshCw, Cpu, Award, Palette, Columns, Maximize, Shield, Layers, Eraser, MoveHorizontal } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
      ...
      <div className="lg:col-span-8 space-y-8">
         <div className="flex items-center justify-between border-b border-white/5 pb-6">
            <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em]">Optic Workspace</h3>
            {processedUrl && (
              <div className="flex bg-white/5 rounded-full p-1 border border-white/5">
                 <button onClick={() => setViewMode('slider')} className={`px-4 py-1.5 rounded-full text-[9px] font-black tracking-widest transition-all ${viewMode === 'slider' ? 'bg-white text-black' : 'text-slate-500 hover:text-white'}`}>SLIDER</button>
                 <button onClick={() => setViewMode('split')} className={`px-4 py-1.5 rounded-full text-[9px] font-black tracking-widest transition-all ${viewMode === 'split' ? 'bg-white text-black' : 'text-slate-500 hover:text-white'}`}>SIDE-BY-SIDE</button>
              </div>
            )}
         </div>
         ...
         {!imageUrl ? (
           <motion.div 
             ...
             className={`w-full aspect-[4/3] rounded-[48px] border-2 border-dashed flex flex-col items-center justify-center gap-8 cursor-pointer transition-all ${
           ...
           >
              ...
              <div className="text-center space-y-2">
                 <p className="text-xl font-bold text-white">Ingest Raw Visual</p>
                 <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">PNG, JPG, WebP — max 25MB</p>
              </div>
           </motion.div>
         ) : (
           <div className="w-full aspect-[4/3] bg-black/40 rounded-[48px] border border-white/5 overflow-hidden relative flex items-center justify-center p-8">
              {viewMode === 'slider' && processedUrl ? (
                <div 
                  ...
                  className="relative w-full h-full rounded-3xl overflow-hidden cursor-ew-resize border border-white/10"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full h-full items-center">
                   <div className="space-y-4">
                      <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest block text-center">Reference Input</span>
                      <img src={imageUrl} className="w-full h-64 object-contain rounded-2xl bg-white/5 border border-white/5" />
                   </div>
                   {processedUrl && (
                     <div className="space-y-4 animate-in">
                        <span className="text-[9px] font-black text-[#64d2ff] uppercase tracking-widest block text-center">Neural Extraction</span>
                        <div className="w-full h-64 rounded-2xl relative overflow-hidden bg-white/5 border border-white/5" style={{ background: bgColor.startsWith('solid-') ? bgColor.replace('solid-', '') : 'transparent' }}>
      ...
      <div className="lg:col-span-4 space-y-10">
         <div className="glass-card p-10 space-y-10">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {['transparent', '#0c0c0d', '#0071e3', '#30d158'].map((color) => (
                      <button 
                        ...
                        className={`h-12 rounded-2xl border-2 transition-all ${bgColor === (color === 'transparent' ? 'transparent' : `solid-${color}`) ? 'border-white scale-110 shadow-2xl' : 'border-white/5 hover:border-white/10'}`} 
            ...
            <div className="space-y-4 pt-10 border-t border-white/5">
               {!processedUrl ? (
                 <button onClick={runIsolation} disabled={!imageUrl || isProcessing} className="pill-button pill-button-primary w-full h-16 text-base gap-3">
                    {isProcessing ? <RefreshCw className="w-6 h-6 animate-spin" /> : <><Eraser className="w-6 h-6" /> Execute Isolation</>}
                 </button>
               ) : (
                 <div className="space-y-4">
                    <button className="pill-button pill-button-primary w-full h-16 text-base gap-3 shadow-[0_0_40px_rgba(255,255,255,0.1)]">
                       <Download className="w-6 h-6" /> Export Master PNG
                    </button>
                    <button onClick={() => { setImageUrl(''); setProcessedUrl(''); setFile(null); }} className="pill-button pill-button-ghost w-full h-16 text-base">Clear Studio</button>
                 </div>
               )}
            </div>
         </div>
         ...
         <div className="p-8 bg-[#64d2ff]/5 border border-[#64d2ff]/10 rounded-[32px] space-y-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed font-medium">Subject isolation is computed within your local graphics buffer. No image arrays are serialized or transmitted externally.</p>
         </div>
      </div>
